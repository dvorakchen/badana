...
export class AgentService {
	...
	async ping(): Promise<boolean> {
		...
		const baseMessages: ChatCompletionMessageParam[] = [{ role: 'system', content: 'hi' }];
		try {
			await this.openai?.chat.completions.create({
				model: this.model,
				messages: baseMessages,
				max_tokens: 10 // 限制回复长度，进一步省钱
			});

			return true;
		} catch (error: unknown) {
			this.logger.error('❌ Ping AI 连接失败！');

			if (error instanceof OpenAI.APIError) {
				this.logger.error(`状态码: ${error.status}`);
				this.logger.error(`错误类型: ${error.type}`);
